...
session_start();
	...
	if ($_POST["buttonModif"]) 
	{
		...
		$sql ="UPDATE Vendeur SET nom='$nom' WHERE ID='$id'";
		...
		if ($prenom != "") 
		{
		$sql ="UPDATE Vendeur SET prenom='$prenom' WHERE ID='$id'";
		...
		if ($adresse!= "") 
		{
		$sql ="UPDATE Vendeur SET adresse='$adresse' WHERE ID='$id'";
		...
		echo "Modification de l'adresse  Réussie <br>"; 
 		 ...
 		 if ($email!= "") 
		{
		$sql ="UPDATE Vendeur SET email='$email' WHERE ID='$id'";
		...
		if ($password!= "") 
		{	
		$sql ="UPDATE Vendeur SET MdP='$password' WHERE ID='$id'";
